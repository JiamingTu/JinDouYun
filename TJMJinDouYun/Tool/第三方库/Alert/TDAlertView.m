...
- (id)initWithTitle:(NSString *)title message:(NSString *)message items:(NSArray<TDAlertItem *> *)items delegate:(id<TDAlertViewDelegate>)delegate {
    ...
    if (self) {
        ...
        _titleLabel.textColor = TJMFUIColorFromRGB(0x333333);
        _titleLabel.textAlignment = NSTextAlignmentCenter;
        ...
        _collectionView.backgroundColor = TJMFUIColorFromRGB(0xcdcdcd);
...
- (void)layoutSubviews {
    ...
    // title
    {
        ...
        if (_messageLabel.text == nil) {
            _edgeInsets.top = _edgeInsets.bottom = (80 * TJMHeightRatio - titleRect.size.height) / 2;
        }
...
- (id)initWithTitle:(NSString *)title titleColor:(UIColor *)color {
    self = [super init];
    if (self) {
        _title = title;
        _font = [UIFont systemFontOfSize:16];//UI要求字体
        _titleColor = color;
